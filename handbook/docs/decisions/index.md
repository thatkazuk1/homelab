...
| ADR | Decision |
|---|---|
| [0001](0001-forgejo-over-gitea.md) | Forgejo over Gitea for self-hosted git — governance and 2026 community consensus, no material technical downside |
| [0002](0002-komodo-for-docker-gitops.md) | Komodo for Docker-level GitOps — topology fit for a multi-host, non-clustered Docker fleet |
...
| [0008](0008-per-stack-sops-secrets.md) | Per-stack SOPS-encrypted secrets, decrypted at deploy time — one `secrets.enc.env` per stack, uniform wrapper, no plaintext on disk |
| [0009](0009-flat-stack-layout.md) | Flat stack layout (`stacks/<name>/`, not `stacks/<host>/<name>/`) — host binding lives in Komodo's own data model, not the git path |
| [0013](0013-handbook-location.md) | Handbook source location and serving — source is public and tracked from day one; serving stays LAN-internal until a public-presentation decision is made |
...
as everything else meant to be shared.
